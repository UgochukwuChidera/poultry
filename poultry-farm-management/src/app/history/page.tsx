...
export default async function HistoryPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  if (state.status === "config_error") {
    return <p className="rounded bg-red-100 p-3 text-red-900">Set Supabase URL/key env vars (or SUPABASE_DB_URL fallback) and run Supabase migrations to view history.</p>;
  }
